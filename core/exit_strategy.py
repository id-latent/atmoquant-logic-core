...
# ==============================================================================
"""
AQL Exit Strategy (FIXED)

Fixes:
  EXIT-1: update_prices() sekarang berjalan concurrent (asyncio.gather)
          untuk efisiensi — sebelumnya sequential per posisi
  EXIT-2: _execute_exit() yang gagal sekarang di-retry 1x setelah 2 detik
          sebelum menyerah, agar sell yang gagal karena timeout tidak
          meninggalkan posisi terbuka tanpa usaha ulang
...
class ExitStrategy:
    ...
    async def _execute_exit(
        ...
        FIX EXIT-2: Retry 1x setelah 2 detik jika sell gagal.
        Returns ExitResult atau None jika semua attempt gagal.
        """
        ...
        try:
            ...
            if receipt is None:
                # FIX EXIT-2: Retry 1x sebelum menyerah
                log.warning(
                    "[Exit] Sell gagal untuk %s — retry dalam 2 detik",
                    pos.position_id,
                )
                ...
                if receipt is None:
                    log.error(
                        "[Exit] Sell tetap gagal setelah retry untuk %s",
                        pos.position_id,
                    )
                    return None
    ...
    async def update_prices(
        ...
        # FIX EXIT-1: Update harga secara concurrent — lebih cepat dari sequential
        async def _update_one(pos) -> None:
            try:
                new_price = await gamma.refresh_market_price(pos.market_id)
                if new_price is not None:
                    self._tracker.update_price(pos.position_id, new_price)
            except Exception as e:
                log.debug("[Exit] Price update failed %s: %s", pos.position_id, str(e))

        await asyncio.gather(*[_update_one(p) for p in open_positions])
