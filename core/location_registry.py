# ==============================================================================
# core/location_registry.py — Global City Registry (FIXED)
# ==============================================================================
"""
...
Fixes:
  BUG #11 : Canada dikategorikan sebagai "Other" sehingga menerima
            Golden Hour window yang sama dengan kota-kota miscellaneous.
            Sekarang Canada menjadi region tersendiri "Canada" dengan
            Golden Hour window yang dikonfigurasi di settings.py.
            Secara timezone, kota-kota Canada sangat mirip US (EST/PST),
            sehingga window yang dipakai = GOLDEN_HOUR_US sebagai default.
  BUG #17 : Doha menggunakan "Asia/Qatar" — ini valid di pytz, tapi
            diverifikasi dan dikonfirmasi tetap valid di pytz 2024.1.
            Tidak ada perubahan kode, hanya catatan konfirmasi.

Catatan:
  - "Asia/Qatar" → valid, UTC+3 permanen (Qatar tidak pakai DST)
  - Semua timezone divalidasi ulang terhadap pytz 2024.1
"""
...
_RAW: dict[str, tuple] = {
    ...
    # ==========================================================================
    # CANADA — FIX BUG #11: region diubah dari "Other" ke "Canada"
    # Golden Hour window untuk Canada = GOLDEN_HOUR_US (lihat settings.py)
    # karena pola perdagangan Polymarket untuk kota Canada sangat mirip US
    # ==========================================================================
    "toronto":       (43.6532, -79.3832, "America/Toronto",     "Canada", "C", 2),
    "yyz":           (43.6777, -79.6248, "America/Toronto",     "Canada", "C", 2),
    "vancouver":     (49.2827,-123.1207, "America/Vancouver",   "Canada", "C", 2),
    "montreal":      (45.5017, -73.5673, "America/Toronto",     "Canada", "C", 2),
    "calgary":       (51.0447,-114.0719, "America/Edmonton",    "Canada", "C", 3),
    ...
    # BUG #17: "Asia/Qatar" dikonfirmasi valid di pytz 2024.1 (UTC+3, no DST)
...
def resolve_location(question: str) -> Optional[CityInfo]:
    ...
    LRU cache (maxsize=512) — pertanyaan sama tidak perlu loop ulang.
    """
    q = question.lower()
...
def detect_unit(question: str, city: CityInfo) -> str:
    ...
    Priority:
      1. Explicit °F atau FAHRENHEIT → F
      2. Explicit °C atau CELSIUS → C
...
def calculate_min_liquidity(
    ...
    """
    Hitung minimum liquidity secara dinamis berdasarkan:
