...
def _select_binary_token(
    ...
    """
    Pilih token dan signal yang benar untuk binary market.
    Guard: jika BUY_NO tapi no_token_id kosong → downgrade ke NO_TRADE.

    Returns: (token_id, signal_str)
    """
    if signal_str == "BUY_NO" and not no_token_id:
        log.warning(
            "[Probability] BUY_NO tapi no_token_id kosong%s — downgrade ke NO_TRADE",
            f" untuk {context[:50]}" if context else "",
        )
        return yes_token_id, "NO_TRADE"
    if signal_str == "BUY_YES":
        return yes_token_id, signal_str
    if signal_str == "BUY_NO":
        return no_token_id, signal_str
    return yes_token_id, "NO_TRADE"  # NO_TRADE — token tidak dipakai
...
def evaluate_binary(
    ...
    if range_result is not None:
        ...
        # FIX BUG #4 + guard empty token: _select_binary_token()
        token_id, signal_str = _select_binary_token(
            signal_str, yes_token_id, no_token_id,
            context=f"RANGE {low_c:.1f}-{high_c:.1f}°C",
        )
    ...
    token_id, signal_str = _select_binary_token(
        signal_str, yes_token_id, no_token_id,
        context=f"{mtype.value} {threshold_c:.1f}°C",
    )
