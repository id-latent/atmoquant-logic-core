# ==============================================================================
# gamma_client.py — v2.1.0
# ==============================================================================
"""
AQL Gamma Client — Lapisan API Polymarket + Eksekusi CLOB

Perubahan dari v2.0.0:
1. Tambah dataclass OutcomeInfo — outcomes sebagai object, bukan dict
2. Tambah dataclass TemperatureMarket — PolyMarket + enrichment kota & golden hour
3. Tambah GammaClient.enrich_markets() — konversi PolyMarket → TemperatureMarket
4. discover_temperature_markets() kini return list[TemperatureMarket]
5. CLOBExecutor.submit_order() signature diubah agar cocok dengan engine.py:
   submit_order(token_id, size_usd, ask_price, slippage_pct)
6. GammaClient.unknown_markets diinisialisasi di __init__
"""
...
class OutcomeInfo:
    """
    Satu outcome dari market MULTI_OUTCOME.
    Dikonversi dari dict mentah di PolyMarket.outcomes oleh enrich_markets().
    """
    label:        str
    token_id:     str
    ...
    condition_id: str
    liquidity:    float = 0.0
    volume_24h:   float = 0.0  # estimasi: volume_usd event / jumlah outcome


# ── TemperatureMarket — PolyMarket + enrichment ───────────────────────────────

@dataclass
class TemperatureMarket:
    """
    PolyMarket yang sudah diperkaya dengan:
    - CityInfo (kota + koordinat + tier + region)
    - Golden Hour status & multiplier
    - cache_key & event_slug untuk tracking

    Dibuat oleh GammaClient.enrich_markets().
    Inilah yang dipakai oleh engine.py.
    """

    # ── Field dari PolyMarket ──────────────────────────────────────────────
    market_id:    str
    condition_id: str
    question:     str
    description:  str
    ...
    # Multi-outcome — sudah dikonversi ke OutcomeInfo
    outcomes: list = field(default_factory=list)  # list[OutcomeInfo]

    event_id: Optional[str] = None

    # ── Field hasil enrichment ─────────────────────────────────────────────
    city:               Optional[object] = None   # CityInfo dari location_registry
...
class GammaClient:
    """
    Wrapper async untuk Polymarket Gamma API — discovery terpadu v2.1.0.

    discover_temperature_markets() sekarang return list[TemperatureMarket]
    (sudah diperkaya dengan kota + golden hour), bukan list[PolyMarket].

    Market yang kotanya tidak dikenal dikumpulkan di self.unknown_markets
    untuk dikirim ke Discord via notifier.notify_unknown_city().
    """
    ...
    def enrich_markets(
        ...
        """
        Konversi list[PolyMarket] → list[TemperatureMarket].

        Untuk setiap market:
        1. Ekstrak nama kota dari pertanyaan via resolve_location()
        2. Hitung golden hour status + multiplier
        3. Skip jika SKIP (di luar jendela trading)
        4. Konversi outcomes dict → list[OutcomeInfo]
        5. Bangun cache_key dan event_slug
        ...
        for pm in raw_markets:
            ...
            # 3. Konversi outcomes dict → OutcomeInfo
...
class CLOBExecutor:
    ...
    Perubahan v2.1.0:
    submit_order(token_id, size_usd, ask_price, slippage_pct=0.02)
    — tidak lagi menerima market/side/outcome_token_id sebagai parameter.
    Token ID dan harga ditentukan di engine.py sebelum memanggil ini.
    """
    ...
    async def submit_order(
        ...
        Parameter:
          token_id    : token ID yang akan dibeli (YES, NO, atau outcome spesifik)
          size_usd    : ukuran posisi dalam USD
          ask_price   : harga ask yang akan dibeli
          slippage_pct: toleransi slippage (default 2%)
    ...
    async def sell_position(
        ...
        Harga jual = current_price dengan slippage.
        """
        ...
        order       = {
            ...
            "size":       str(round(size_usd / entry_price, 4)),
            "nonce":      int(time.time() * 1000),
            "feeRateBps": "170",
            "expiration": "0",
            "maker":      self._account.address,
